...
export default function DailySalesChart({ apiBaseUrl }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await authFetch(`${apiBaseUrl}/api/receipts/analytics/daily?tz=${encodeURIComponent(tz)}`);
        if (response.ok) {
  ...
  return (
    <div className="border border-[var(--line)] bg-[var(--bg-primary)] p-4 h-64 flex flex-col">
      <h3 className="text-xs font-semibold uppercase tracking-wider mb-4 text-[var(--ink-soft)]">Daily Velocity (14 Days)</h3>
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
            ...
            />
            <Line yAxisId="left" type="monotone" dataKey="revenue" stroke="var(--accent-red)" strokeWidth={2} dot={{ r: 3, fill: 'var(--bg-primary)', stroke: 'var(--accent-red)' }} name="Revenue" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
